...
# include("./adm-MTK/MTK_Advection.jl"); #MTK_Advection
# include("./adm-MTK/Dikes.jl");
# include("./adm-MTK/MTK_Dikes.jl");
# include("./adm-MTK/MTK_Tracers.jl")
...
@views function DikeInjection_2D();
    ...
    figdir = "figs2D_low_visc_magma"
  # Standard MTK Toba routine--------------------------------------------------------------

   # nondimensionalize with CharDim 
    CharDim    =    GEO_units(length=100km, viscosity=1e22Pa*s)
...
# ----------CrossSections for 2D simulation----------------------------------
    ...
    Data_Cross              = CrossSection(DataPara, dims=(nx+1,ny+1), Interpolate=true,Start=(ustrip(Grid.min[1]),ustrip(Grid.max[2])), End=(ustrip(Grid.max[1]), ustrip(Grid.min[2])))
...
#-------JustRelax parameters----------------------------------------------------------------
    ...
    dike                    = MagmaThermoKinematics.Dike(W=W_in, H=H_in, Type="ElasticDike", T=T_in)
    nTr_dike                =   300; 
    ...
    # Tracers, Tnew_cpu, Vol = InjectDike(Tracers, parts_semilagrange, Tnew_cpu, xvi, dike, nTr_dike)   # Add dike, move hostrocks
     ...
     while it < nt
        ...
        if mod(it, 2) == 0
          ...
          dike = MagmaThermoKinematics.Dike(;
              Angle=[Angle_rand],
              Center=cen,
              W=W_in,
              H=H_in,
          ...
          @copy Tnew_cpu Array(thermal.T)
          ## MTK injectDike
          Tracers, Tnew_cpu, Vol   =   MagmaThermoKinematics.InjectDike(Tracers, Tnew_cpu, xvi, dike, nTr_dike,  AdvectionMethod="RK2");   # Add dike, move hostrocks
          ## AdM injectDike
        ...
        #   copy_arrays_GPU2CPU!(Tnew_cpu, Phi_melt_cpu, thermal.T, ϕ)     # Copy arrays to CPU to update properties
          UpdateTracers_T_ϕ!(Tracers, xvi, Tnew_cpu, Phi_melt_cpu);      # Update info on tracers 
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            # ϕ_d = ustrip.(dimensionalize(Array(ϕ), Pas, CharDim))
            ...
            p5 = Plots.heatmap(x_v, y_v, ϕ', aspect_ratio=1, xlims=(minimum(x_v), maximum(x_v)), ylims=(minimum(y_v), maximum(y_v)), c=:jet,
                                    title="Melt fraction", xlabel="width [km]")

            Plots.plot(p1, p2 #=, p3, p4=#, p5, layout=(3, 1), size=(1200,1200))
